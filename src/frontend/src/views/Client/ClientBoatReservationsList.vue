<template>
  ...
  <div class="header">
    ...
    <!-- sort -->
    <template v-if="!upcomingReservations && !availableQuickReservations && boatReservationDtos.length!=0">
      <hr />
      <form>
        <h1 style="text-align: left; color: #0b477b; padding-left: 7.2%">
          Sort boat reservations
        </h1>
        <br />
        <div
    ...
    <div class="carousel-inner py-4">
      ...
      <div class="carousel-item active">
        <div class="container">
          <div class="row">
            <div
              v-for="(boatReservationDto, index) in sortedReservations"
              :key="index"
              class="col-lg-4"
            >
                  ...
                  <div class="row">
                    <div class="col-10">
                      <h2 style="text-align: left" class="card-title">
                        {{ boatReservationDto.boatDto.name.toUpperCase() }}
                      </h2>
                    </div>
                    <div class="col-10">
                    ...
                    <div style="vertical-align: bottom" class="col">
                      <span class="badge bg-warning text-light"
                        ...
                        {{ boatReservationDto.boatDto.rating }}</span
                      >
                    </div>
                  </div>
                  <hr />
                      ...
                      <div class="col" style="text-align: right">
                        ...
                        <template
                          v-if="
                            ...
                            !boatReservationDto.evaluated
                          "
                        >
                          <button
                            @click="evaluateReservation(boatReservationDto)"
                            type="button"
                            class="btn btn-outline-dark rounded-pill"
                          >
                            EVALUATE
    ...
    <div class="row">
      <div class="col">
        <div class="row">
          ...
          <div class="col-sm-9" style="padding: 1%; text-align: left">
            <p>
              <b>{{ boatForCancellation.boatDto.name }}</b>
            </p>
          </div>
        </div>
        <div class="row">
...
export default {
  ...
  methods: {
    ...
    getFullAddress: function (index) {
      if (this.reservationsLoaded == true)
        return (
          this.sortedReservations[index].boatDto.addressDto.streetAndNum +
          ", " +
          this.sortedReservations[index].boatDto.addressDto.city +
          ", " +
          this.sortedReservations[index].boatDto.addressDto.country
        );
      return "logoF1.png";
    },
    ...
    writeComplaint: function (reservationDto) {
      this.$router.push(
        "/complaint/" + this.email + "/" + "boat/" + reservationDto.id
      );
    },
    cancelReservationModal: function (reservationDto) {
      this.boatForCancellation = reservationDto;
      ...
      axios
        ...
        .then(() => {
          ...
          this.$swal.fire({
            position: "top-end",
            icon: "success",
            title: "Boat reservation cancellation successful!",
            showConfirmButton: false,
            timer: 2500,
          });
          this.getReservations();
    ...
    sortedReservations: function () {
      if (this.upcomingReservations || this.availableQuickReservations)
        return this.boatReservationDtos;
      {
        return this.sortedArray;
      }
    },
    sortedArray: function () {
      let sortedEntities = this.boatReservationDtos;
