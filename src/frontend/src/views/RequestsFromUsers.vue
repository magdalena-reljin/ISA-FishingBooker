...
<table class="table">
  ...
  <tbody>
    <tr v-for="(user,index) in userRequestDtos" :key="index">
      <th scope="row">1</th>
      <td>{{user.role}}</td>
      <td>{{user.firstname}}</td>
      <td>{{user.lastname}}</td>
      <td>{{user.email}}</td>
      <td><button @click="accept()"  type="button" class="btn btn-outline-success">ACCEPT</button>
                          <button  type="button" class="btn btn-outline-danger" >DENY</button></td>
...
import axios from "axios";
   export default{
     ...
     mounted(){
           axios
               .get("http://localhost:8081/auth/getNewUsers")
               .then((response) => {
                   this.userRequestDtos=response.data;
