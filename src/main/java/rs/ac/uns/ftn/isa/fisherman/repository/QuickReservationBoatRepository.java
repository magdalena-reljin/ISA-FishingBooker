...
public interface QuickReservationBoatRepository extends JpaRepository<QuickReservationBoat,Long> {
    ...
    @Query(value="SELECT * FROM quick_reservation_boat res join boat b on res.boat_id=b.id where b.users_id=:users_id ",nativeQuery = true)
    Set<QuickReservationBoat> findReservationsByOwnerId(@Param("users_id")Long ownerId);
